...
def find_projective_correspondence(source_points,
    ...
    #define conditions for where the points in the map are 
    mask = (target_us >= 0) & (target_us < w) & (target_vs >= 0) & (target_vs < h) & (dist_diff >= 0) 
    # End of TODO
...
def build_linear_system(source_points, target_points, target_normals, T):
    ...
    # print('n_q shape: ',n_q.shape)
    A = np.zeros((M, 6))
    b = np.zeros((M, ))
    for i in range(M):
    # TODO: build the linear system
        A[i,0] = -n_q[i,1]*p_prime[i,2] + n_q[i,2]*p_prime[i,1]
        A[i,1] = n_q[i,0]*p_prime[i,2] - n_q[i,2]*p_prime[i,0]
        A[i,2] = -n_q[i,0]*p_prime[i,1] + n_q[i,1] * p_prime[i,0]
        A[i,3] = n_q[i,0]
        A[i,4] = n_q[i,1]
        A[i,5] = n_q[i,2]

        b[i] = n_q[i,0]*(p_prime[i,0]-q[i,0]) + n_q[i,1]*(p_prime[i,1]-q[i,1]) + n_q[i,2]*(p_prime[i,2]-q[i,2])
    # b = n_q @ (p_prime - q)
    # print('A',A.shape)
    # print('b',b.shape)
    # End of TODO
...
def solve(A, b):
    ...
    #paste from homework 3: Use from homework 3
    #Using qr function
