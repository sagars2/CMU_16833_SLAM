...
class Map:
    ...
    def merge(self, indices, points, normals, colors, R, t):
        ...
        alpha = 1
        T_p = (R @ points.T + t).T
        normal_T = (R @ normals.T).T
        
        self.points[indices] = (self.weights[indices] * self.points[indices] + T_p)/(self.weights[indices] + alpha)
        self.normals[indices] = (self.weights[indices] * self.normals[indices] + normal_T)/(self.weights[indices] + alpha)
        self.normals[indices] = self.normals[indices]/np.linalg.norm(self.normals[indices], axis=1, keepdims=True)
        self.colors[indices] = (self.weights[indices] * self.colors[indices] + colors)/(self.weights[indices] + alpha)
    ...
    def add(self, points, normals, colors, R, t):
        ...
        T_p = (R @ points.T + t).T
        normal_T = (R @ normals.T).T
        self.points = np.vstack((self.points, T_p))
        self.normals = np.vstack((self.normals,normal_T))
    ...
    def filter_pass2(self, points, normals, input_points, input_normals,
        ...
        p1 = points
        q1 = input_points
        n1 = np.linalg.norm((p1 - q1),axis=1)
        # norm_dot = normals @ input_normals
        normals = normals/np.linalg.norm(input_normals,axis=1,keepdims=True)
        norm_dot = np.sum(normals*input_normals,axis=1)
        # normal_of_/normals
        angle_mask = abs(np.arccos(norm_dot)) < angle_diff
        mask = (n1<dist_diff) & (angle_mask)
        return mask
