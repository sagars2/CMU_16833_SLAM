...
def create_linear_system(odoms, observations, sigma_odom, sigma_observation,
    ...
    #Defining all the Jacobians
    J_odom = np.asarray([[-1,0,1,0],[0,-1,0,1]],dtype=object)
    J_land = np.asarray([[-1,0,1,0],[0,-1,0,1]],dtype=object)

    # TODO: Then fill in odometry measurements
    A_odom = sqrt_inv_odom @ J_odom
    for i in range(n_odom):
        A[2+2*i:2*i+4,2*i:2*i+4] = A_odom
        b[2+2*i:2*i+4] =  sqrt_inv_odom @ odoms[i]

    # TODO: Then fill in landmark measurements
    A_land = sqrt_inv_obs @ J_land
    
    for j in range(n_obs):
        pose_indices = (observations[j,0]).astype(int)
        landmark_indices = (observations[j,1]).astype(int)
        measurement = observations[j,2:]
        
        A[(n_odom+1)*2 + 2*j:(n_odom+1)*2 + 2*(j+1),pose_indices*2:2*(pose_indices+1)] = A_land[:,:2]
        A[(n_odom+1)*2 + 2*j:(n_odom+1)*2 + 2*(j+1),2*(n_poses+landmark_indices):2*(n_poses+landmark_indices+1)] = A_land[:,2:]
        b[(n_odom+1)*2 + 2*j:(n_odom+1)*2 + 2*(j+1)] = sqrt_inv_obs @ measurement
